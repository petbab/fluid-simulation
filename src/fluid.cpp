...
Fluid::Fluid(unsigned grid_count, float gap, BoundingBox bounding_box)
    ...
    geometry = AssetManager::make<InstancedGeometry>(
        ...
        std::vector{VertexAttribute{3, vec_to_span(positions)}});
}
...
void Fluid::update(double delta) {
    ...
    dynamic_cast<const InstancedGeometry*>(geometry)->update_instance_data(vec_to_span(positions));
}
...
void Fluid::initiate_particles(const unsigned grid_count, const float gap) {
    assert(grid_count > 1);
